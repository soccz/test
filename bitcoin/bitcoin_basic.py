...
ax.plot(dates, prices, label='History (Past 200 Days)', color='blue')

# 2. 모델 예측선: 전체적인 상승/하락 추세 (Trend Line)
ax.plot(dates, model.predict(X), label='Linear Trend Line', color='green', linestyle='--', alpha=0.7)

# 3. 미래 예측: 다음 날 예상 가격 (Prediction Point)
ax.scatter([next_date], [predicted_price], color='red', s=100, label='Next Day Prediction', zorder=5)

# X축 날짜 포맷 설정
ax.xaxis.set_major_locator(mdates.MonthLocator())  # 월별로 주요 눈금
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))  # 날짜 형식
ax.xaxis.set_minor_locator(mdates.WeekdayLocator())  # 주별로 보조 눈금
plt.xticks(rotation=45)  # 날짜 레이블 회전

ax.set_title(f"Bitcoin Prediction - Basic (200 Days -> Next Day) | {datetime.now().strftime('%Y-%m-%d')}")
ax.set_xlabel("Date")
